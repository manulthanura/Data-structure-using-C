...
// At this time, we give the direction of Bug faces
...
char world[8][8] = {{'x',' ',' ',' ',' ',' ',' ','x'},
                    ...
                    {' ',' ',' ',' ','x',' ',' ',' '},
                    {'x','x','x',' ','x','x','x','x'}};
int x_b,y_b,x_f,y_f,rot_st ;  //bug positions, rotation and food position
int conf_b,conf_f,conf_r,next,win =0; //confirming variables
char bug = 2 ;
char food = 4;
...
void main_menu(){
    ...
    printf("Bug - %c  || Food - %c\n",bug,food);
    printf("Rotation Stats : \n");
    printf("Up - press 1 || Right - press 2 || Down - press 3 || Left - press 4\n\n");
...
void start(){
    ...
    while(conf_r != 1){
        printf("Enter Rotation state : ");
        scanf("%d", &rot_st);

        if(rot_st < 5 && rot_st > 0){
            conf_r =1;
        }else {
            printf("Invalid rotation state.\n");
        }
    }
}
